...
def generate_fixed_trajectory( n_points=21, step_size=2):
    ...
    x=0 
    z=-5
    style = random.choice(["ligne_droite_descendante","ligne_droite_montante","spirale", "zigzag", "montante", "descendante"])
    if style == "ligne_droite_descendante":
        for _ in range(n_points):
            trajectory.append((x, y, z))
            x += step_size
            y=y-1
    elif style == "ligne_droite_montante":
           y=random.randint(-10,-6)
           for _ in range(n_points):
             trajectory.append((x, y, z))
             x += step_size
             y=y+1
        ...
        for i in range(n_points):
            ...
            y = radius * math.sin(angle)+i*0.5
            z = -5 +    i  # monte doucement
            trajectory.append((x, y, z))
...
file_exists = os.path.isfile("false_flights_mission.csv")

with open("false_flights_mission.csv", "a", newline='') as f:
    writer = csv.writer(f)
